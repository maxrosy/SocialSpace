...
def main():
    ...
    main_parser.add_argument('--from', help='start time(included), format: YYYY-mm-dd HH:MM:SS')
    main_parser.add_argument('--to', help='end time(not included), format: YYYY-mm-dd HH:MM:SS')
    ...
    # Add Weixin Search Content function command line
    weixin_search_subparser = weixin_subparser.add_parser('search_content', parents=[main_parser])
    weixin_search_subparser.set_defaults(app='weixin',function='search_content')

    # Add Weixin Article Content function command line
    ...
    weixin_article_subparser.set_defaults(app='weixin',function='article_content')
    ...
    newrank = NewRankAPI()
    if opt.get('app') == 'weixin' and opt.get('function') == 'search_content':
        newrank.get_weixin_data_combine_search_content(from_time,to_time,**opt)
    elif opt.get('app') == 'weixin' and opt.get('function') == 'article_content':
        account = opt.pop('account')
